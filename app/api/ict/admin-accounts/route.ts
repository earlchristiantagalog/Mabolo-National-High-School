...
async function sendAccountEmail(
  ...
  const mailOptions = {
    ...
    to: email,
    subject: `MNHS Admin Account - ${role}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: #f9f9f9; padding: 30px; border: 1px solid #ddd; border-radius: 0 0 8px 8px;">
          <h2 style="color: #8B1010; margin-top: 0;">Admin Account Created!</h2>
          <p>Hello ${firstName},</p>
          <p>Your admin account (<strong>${role}</strong>) has been created. Below are your login credentials.</p>
          <div style="background-color: white; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #eee;">
            ...
            <table style="width: 100%; font-size: 13px; color: #333;">
              <tr><td style="padding: 5px 0; color: #666; width: 120px;">Admin ID</td><td style="padding: 5px 0; font-weight: bold; color: #8B1010; font-size: 16px; letter-spacing: 1px;">${accountId}</td></tr>
          ...
          <div style="background-color: #fff3cd; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #ffc107;">
            <p style="margin: 0; font-size: 13px; color: #856404;"><strong>Important:</strong> Please keep your Admin ID and Password secure.</p>
          </div>
...
export async function GET() {
  try {
    ...
    const admins = await query<{
      ...
      `SELECT id, account_id, first_name || ' ' || last_name AS name, role, email, status
       FROM admin_accounts
  ...
  try {
    ...
    while (attempts < 10) {
      const dup = await query<{ id: number }[]>(
        "SELECT id FROM admin_accounts WHERE account_id = $1",
        [accountId]
      );
    ...
    await query(
      "INSERT INTO admin_accounts (first_name, last_name, role, email, account_id, password) VALUES ($1, $2, $3, $4, $5, $6)",
      [firstName, lastName, role, email, accountId, password]
    );

    try {
      await sendAccountEmail(email, firstName, role, accountId, password);
    } catch (emailError) {
      console.error("Admin account email send failed:", emailError);
    }
