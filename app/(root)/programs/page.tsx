          ...
          <div className="flex items-center justify-between h-24">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {[{ label: "About", href: "/about" }, { label: "Programs", href: "/programs" }, { label: "Admission", href: "/admission" }, { label: "Announcements", href: "/news" }, { label: "Contact", href: "/contact" }].map((item) => (
                <Link key={item.href} href={item.href} className={`px-4 py-2 text-sm font-medium transition-colors rounded ${item.href === "/programs" ? "text-[#8B1010] font-bold" : "text-gray-700 hover:text-[#8B1010] hover:bg-red-50"}`}>{item.label}</Link>
              ))}
      ...
      <section className="relative h-[300px] bg-cover bg-center flex items-center" style={{ backgroundImage: "url('/bg.jpg')" }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="max-w-2xl">
            <p className="text-[#D4A017] font-semibold text-xs tracking-[0.2em] uppercase mb-3">Academic Programs</p>
            <h2 className="text-4xl font-bold text-white leading-tight">Curriculum Offerings</h2>
          </div>
        </div>
      </section>
      ...
      <div className="bg-gray-100 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span className="text-[#8B1010] font-medium">Programs</span>
          </div>
        </div>
      </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <p className="text-[#D4A017] font-semibold text-xs tracking-[0.2em] uppercase mb-2">K-12 Curriculum</p>
            <h3 className="text-3xl font-bold text-[#8B1010]">Junior & Senior High School</h3>
            <div className="w-16 h-1 bg-[#D4A017] mx-auto mt-4 rounded-full" />
          </div>
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="border border-gray-200 rounded-lg p-8 bg-[#f8f9fa]">
              <h4 className="text-xl font-bold text-[#1E5631] mb-4">Junior High School (JHS)</h4>
              <p className="text-sm text-gray-600 mb-4">Grades 7 to 10 — Core Curriculum following the K-12 Basic Education Program.</p>
              <p className="text-sm text-gray-600 mb-4">Subjects include:</p>
              <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                {["English", "Mathematics", "Science", "Filipino", "Araling Panlipunan", "MAPEH", "Edukasyon sa Pagpapakatao", "Technology & Livelihood Ed."].map((s) => (
                  <div key={s} className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-[#1E5631]" />{s}</div>
                ))}
              </div>
            </div>
            <div className="border border-gray-200 rounded-lg p-8 bg-[#f8f9fa]">
              <h4 className="text-xl font-bold text-[#8B1010] mb-4">Senior High School (SHS)</h4>
              <p className="text-sm text-gray-600 mb-4">Grades 11 to 12 — Specialized tracks under the K-12 Program.</p>
              <p className="text-sm text-gray-600 font-semibold mb-3">Academic Tracks:</p>
              <div className="space-y-2 mb-4">
                {["STEM — Science, Technology, Engineering & Math", "ABM — Accountancy, Business & Management", "HUMSS — Humanities & Social Sciences", "GAS — General Academic Strand"].map((s) => (
                  <div key={s} className="flex items-center gap-2 text-sm text-gray-600"><span className="w-1.5 h-1.5 rounded-full bg-[#8B1010]" />{s}</div>
                ))}
              </div>
              <p className="text-sm text-gray-600 font-semibold mb-3">TVL Tracks:</p>
              <div className="space-y-2">
                {["CSS — Computer Systems Servicing", "TD — Technical Drafting", "HD — Hardware Servicing", "COOKERY — Food & Beverage Services"].map((s) => (
                  <div key={s} className="flex items-center gap-2 text-sm text-gray-600"><span className="w-1.5 h-1.5 rounded-full bg-[#D4A017]" />{s}</div>
                ))}
              </div>
            </div>
          </div>
